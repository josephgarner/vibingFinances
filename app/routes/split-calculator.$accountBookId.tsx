...
type SplitRow = {
  name: string;
  totalAmount: number;
  splits: { label: string; percent: number }[];
};
...
export default function SplitCalculator() {
  ...
  const [globalSplits, setGlobalSplits] = useState<
    { label: string; percent: number }[]
  >([
    { label: "Person 1", percent: 50 },
    { label: "Person 2", percent: 50 },
  ]);
          ...
          prev.map((l) =>
            l.id === selectedListId
              ? {
                  ...
                  name,
                  items: rows,
                  updatedAt: (fetcher.data as any).updatedAt || l.updatedAt,
                }
              : l
          )
        );
  ...
  const totals = useMemo(() => {
    return rows.map((row) => {
      const clampedSplits = row.splits.map((s) => ({
        ...s,
      ...
      return {
        ...
        splits: clampedSplits.map((s) => ({
          label: s.label,
          percent: s.percent,
          amount: (row.totalAmount * s.percent) / 100,
        })),
        percentTotal: sum,
      };
    });
  ...
  const synchronizeRowsWithGlobal = (
    splits: { label: string; percent: number }[]
  ) => {
    setRows((prev) =>
      prev.map((r) => ({ ...r, splits: splits.map((s) => ({ ...s })) }))
    );
  };
  const handleAddRow = () =>
    setRows([
      ...rows,
      { name: "", totalAmount: 0, splits: globalSplits.map((s) => ({ ...s })) },
    ]);
  const handleAddSplitGlobal = () => {
    const next = [
      ...globalSplits,
      { label: `Person ${globalSplits.length + 1}`, percent: 0 },
    ];
  ...
  const handleChangeSplitGlobal = (
    ...
    (next[index] as any)[field] =
      field === "percent" ? Number(value) : String(value);
    setGlobalSplits(next);
    synchronizeRowsWithGlobal(next);
  };
            ...
            <Card shadow="sm" radius="md" padding="lg">
              <Stack gap="md">
                <Group>
                  ...
                  <Button color="red" onClick={del} disabled={!selectedListId}>
                    Delete
                  </Button>
                </Group>
                ...
                <Stack gap={6}>
                  <Group justify="space-between" align="center">
                    ...
                    <Button variant="subtle" onClick={handleAddSplitGlobal}>
                      + Add split
                    </Button>
                  </Group>
                  {globalSplits.map((s, si) => (
                    <Group key={si} gap="xs">
                      ...
                      <NumberInput
                        ...
                        value={s.percent}
                        onChange={(v) =>
                          handleChangeSplitGlobal(si, "percent", Number(v || 0))
                        }
                      />
                      <Button
                        color="red"
                        onClick={() => handleRemoveSplitGlobal(si)}
                      >
                        Remove
                      </Button>
                    </Group>
                      ...
                      <Table.Tr key={ri}>
                        <Table.Td>
                          <TextInput
                            ...
                            onChange={(e) => {
                              const copy = [...rows];
                              copy[ri].name = e.currentTarget.value;
                              setRows(copy);
                            }}
                        ...
                        <Table.Td>
                          <NumberInput
                            ...
                            onChange={(v) => {
                              const copy = [...rows];
                              copy[ri].totalAmount = Number(v || 0);
                              setRows(copy);
                            }}
                          />
                        </Table.Td>
                ...
                <Select
                  ...
                  onChange={(v) => {
                    ...
                    if (found) {
                      ...
                      const firstSplits =
                        (found.items?.[0]?.splits as any) || [];
                      if (firstSplits.length > 0) {
                        setGlobalSplits(
                          firstSplits.map((s: any) => ({
                  ...
                  {lists.map((l) => (
                    <Card
                      ...
                      onClick={() => {
                        ...
                        setName(l.name);
                        setRows(l.items as any);
                        const fs = (l.items?.[0]?.splits as any) || [];
                        if (fs.length > 0)
                          setGlobalSplits(
                            fs.map((s: any) => ({
                              label: String(s.label),
